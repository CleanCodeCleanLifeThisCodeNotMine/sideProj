...
(function() {
    ...
    function setupInputClickHandler(inputBox, answer) {
        inputBox.addEventListener('click', function() {
            ...
            // Create and dispatch an input event to notify the website of the change
            const event = new Event('input', { bubbles: true });
            this.dispatchEvent(event); // Dispatch the input event
    ...
    function findContinueButton() {
        if (node.nodeType === Node.TEXT_NODE) {
            // Check if the node contains "Hoan hô! Bạn đã trả lời đúng:"
            let text1 = node.textContent1;
            let resultMatch1 = text1.match(/Hoan hô! Bạn đã trả lời đúng/);
    }
